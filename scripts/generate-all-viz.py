...
"""
批量生成可视化 HTML 文件 - 完整版
从 lessons.ts 中提取题目数据并生成对应的动画演示文件
"""
...
def read_lessons_file():
    """读取 lessons.ts 并提取题目数据"""
    lessons_path = Path('src/data/lessons.ts')
    if not lessons_path.exists():
        print("lessons.ts not found!")
        return []

    content = lessons_path.read_text(encoding='utf-8')

    # 提取 questionsData 数组
    match = re.search(r'const questionsData:\s*Question\[\]\s*=\s*\[(.*?)\];', content, re.DOTALL)
    if not match:
        print("Could not find questionsData array")
        return []
...
def extract_questions(content: str) -> list:
    """从 lessons.ts 内容中提取题目"""
    questions = []

    # 匹配每个题目对象
    pattern = r'\{\s*id:\s*(\d+),\s*chapter:\s*\'([^\']+)\'[\s\S]*?correctAnswer:\s*\'([^\']+)\'[\s\S]*?\}'
    ...
    for match in re.finditer(pattern, content):
        ...
        # 提取更多信息
        id_match = re.search(r'id:\s*(\d+)', match.group(0))
        title_match = re.search(r'title:\s*\'([^\']+)\'', match.group(0))
        code_match = re.search(r'code:\s*`([^`]+)`', match.group(0))
        type_match = re.search(r'type:\s*\'([^\']+)\'', match.group(0))
        desc_match = re.search(r'description:\s*\'([^\']+)\'', match.group(0))
        explanation_match = re.search(r'explanation:\s*\'([^\']+)\'', match.group(0))

        question = {
            'id': int(id_match.group(1)) if id_match else 0,
            'title': title_match.group(1) if title_match else '',
            'type': type_match.group(1) if type_match else 'choice',
            'chapter': chapter,
...
def generate_code_lines(code: str) -> list:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if line.strip().startswith('#'):
            ...
            code_type = 'struct'
        elif '*' in line and ('*' in line.split('=')[0] if '=' in line else False):
            code_type = 'pointer'
...
def generate_steps(code_lines: list, knowledge_point: str, question_type: str = 'code') -> list:
    """生成步骤数据"""
    steps = []
        ...
        if line['type'] == 'declaration':
            ...
            if match:
                ...
                for var_name, var_data in variables.items():
                    ...
                    output_line = output_line.replace('%x', hex(var_data['value']), 1)
                    output_line = output_line.replace('%08b', format(var_data['value'], '08b'), 1)
    ...
    function App() {
      ...
      useEffect(() => {
        if (isPlaying && step < steps.length - 1) {
          const timer = setTimeout(() => setStep(prev => prev + 1), speed);
      ...
      const nextStep = useCallback(() => setStep(prev => Math.min(steps.length - 1, prev + 1)), []);
      ...
      const renderCodeLine = (line, index) => {
        ...
        return (
          <div key={index} className={`code-line ${isCurrent ? 'is-current' : ''} ${isExecuted && !isCurrent ? 'is-executed' : ''}`}>
      ...
      const renderVarCards = () => {
        ...
        if (!vars || Object.keys(vars).length === 0) {
          return <div className="text-center py-8 text-gray-500">暂无变量</div>;
        }
        return (
        ...
        <div className="h-screen flex flex-col overflow-hidden">
          <div className="flex-shrink-0 px-4 py-3 flex items-center justify-between border-b border-white/10"
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="ctrl-btn primary" onClick={togglePlay}>{isPlaying ? '⏸ 暂停' : '▶ 播放'}</button>
          ...
          <div className="flex-1 flex overflow-hidden">
            <div className="flex-[3] flex flex-col border-r border-white/10 overflow-hidden">
              <div className="flex-1 overflow-auto py-3">{codeLines.map(renderCodeLine)}</div>
              <div className="h-28 flex-shrink-0 border-t border-white/10 p-3">
                <div className="terminal h-full overflow-auto">
                  ...
                  {currentStep.output.length === 0 && <div className="text-gray-600 text-sm">等待输出...</div>}
                </div>
              </div>
            </div>
...
def main():
    """主函数"""
    print('Batch generating visualization HTML files...')
    print('批量生成可视化 HTML 文件...')
    print()

    # 示例题目数据
    sample_questions = [
        {'id': 10001, 'title': 'uint8_t 溢出', 'knowledge_point': 'uint8_t 溢出'},
        {'id': 10002, 'title': 'int8_t 溢出', 'knowledge_point': 'int8_t 溢出'},
        {'id': 10003, 'title': '有符号无符号比较', 'knowledge_point': '有符号无符号比较'},
        {'id': 10004, 'title': '位或运算设置位', 'knowledge_point': '位或运算设置位'},
        {'id': 10005, 'title': '位与取反清除位', 'knowledge_point': '位与取反清除位'},
        {'id': 10006, 'title': '位异或翻转位', 'knowledge_point': '位异或翻转位'},
        {'id': 10007, 'title': '位运算提取字段', 'knowledge_point': '位运算提取字段'},
        {'id': 10008, 'title': 'for 循环数组求和', 'knowledge_point': 'for 循环数组求和'},
        {'id': 20001, 'title': '指针基础', 'knowledge_point': '指针基础'},
        {'id': 20002, 'title': '指针与数组', 'knowledge_point': '指针与数组'},
        {'id': 30001, 'title': 'memcpy 内存复制', 'knowledge_point': 'memcpy 内存复制'},
        {'id': 40001, 'title': 'pthread_create', 'knowledge_point': 'pthread_create 线程创建'},
    ]
...
int main() {{
    ...
    print()
    print(f'Done! Generated {generated} visualization files')
